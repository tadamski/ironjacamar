...
....
cp example.rar ${IRONJACAMAR_HOME}/deploy
...
....
copy example.rar ${IRONJACAMAR_HOME}\deploy
...
jakarta.resource.spi.ConnectionEventListener as a connectionErrorOccurred
message
|===
...
`jakarta.resource.spi.ValidatingManagedConnectionFactory` return value.
